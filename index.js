...
// Sketchy.morphologyOperation = function (shape, frameSize, operation) {
//   return 0;
// };
...
// The Hungarian Algorithm This is also published as a standalone library:
// Hungarian.js See: https://github.com/kjkjava/Hungarian.js This version is
// modified to return the function instead of set it as a global variable. This
// algorithm is used in shapeContextMatch to pair up related points with minimum
// cost (which is a normalized distance error).

Sketchy.hungarian = (function () {
  // Expose just the hgAlgorithm method Usage: hungarian(matrix,
  // [isProfitMatrix=false], [returnSum=false])
  // return hgAlgorithm;

  function hgStep1(stepParam, costParam) {
    // For each row of the cost matrix, find the smallest element and subtract it
    // from every other element in its row.

    let minVal;
    const cost = costParam;
    let step = stepParam;
  ...
  function hgStep2(step, cost, mask, rowCover, colCover) {
  ...
  function hgStep3(step, mask, colCover) {
  ...
  function findUncoveredZero(rowCol, cost, rowCover, colCover) {
    let done;
  ...
  function hgStep4(step, cost, mask, rowCover, colCover, zeroRC) {
    // Find an uncovered zero in cost and prime it (if none, go to Step 6). Check
    // for star in same row: if yes, cover the row and uncover the star's column.
    // Repeat until no uncovered zeros are left and go to Step 6.  If not, save
    // location of primed zero and go to Step 5.

    let rowCol = [
      0, 0
    ]; // size: 2, holds row and column of uncovered zero
    let done = false;
    let starInRow;
    ...
    while (!done) {
      ...
      if (rowCol[0] === -1) {
        ...
        starInRow = false;
        for (let j = 0; j < mask[rowCol[0]].length; j++) {
  ...
  function hgStep5(step, mask, rowCover, colCover, zeroRC, path) {
    // Construct series of alternating primes and stars.  Start with prime from step
    // 4.  Take star in the same column.  Next, take prime in the same row as the
    // star.  Finish at a prime with no star in its column. Unstar all stars and
    // star the primes of the series.  Erase any other primes.  Reset covers.  Go to
    // Step 3.
  ...
  function hgStep6(step, cost, rowCover, colCover) {
    // Find smallest uncovered value in cost: a.) Add it to every element of
    // uncovered rows, b.) Subtract it from every element of uncovered columns.  Go
    // to Step 4.

    const minVal = findSmallest(cost, rowCover, colCover);
  ...
  // Takes in a 2D array and finds the largest element This is used in the
  // Hungarian algorithm if the user chooses "max" (indicating their matrix values
  // represent profit) so that cost values are subtracted from the largest value.
  ...
  // Makes a rectangular matrix square by padding it with some value This modifies
  // the matrix argument directly instead of returning a copy
  function makeSquare(matrix, padValue) {
    ...
    // None of the above cases may execute if there is a problem with the input
    // matrix.
  }

    // isProfitMatrix is optional, but if it exists and is the value true, the costs
  // will be treated as profits returnSum is also optional, but will sum up the
  // chosen costs/profits and return that instead of the assignment matrix.
  const hgAlgorithm = function (matrix, isProfitMatrix, returnSum) {
    let maxWeight;
    const mask = []; // the mask array: [matrix.length] x [matrix[0].length]
    const rowCover = []; // the row covering vector: [matrix.length]
    const colCover = []; // the column covering vector: [matrix[0].length]
    const zeroRC = [
      0, 0
    ]; // position of last zero from Step 4: [2]
    const path = []; // [matrix.length * matrix[0].length + 2] x [2]
    let step = 1;
    let done = false;
      // Number.MAX_VALUE causes overflow on profits. Should be larger or smaller than
      // all matrix values. (i.e. -1 or 999999)
    const forbiddenValue = -1;
    const assignments = []; // [min(matrix.length, matrix[0].length)] x [2]
    let assignmentsSeen;

    // Create the cost matrix, so we can work without modifying the original input.
    const cost = copyOf(matrix);

    // If it's a rectangular matrix, pad it with a forbidden value (MAX_VALUE).
    // Whether they are chosen first or last (profit or cost, respectively) should
    // not matter, as we will not include assignments out of range anyway.
    ...
    // Initialize the inside arrays to make 2D arrays Fill with zeros
    for (let i = 0; i < cost.length; i++) {
      mask[i] = [];
      for (let j = 0; j < cost[0].length; j++) {
        mask[i][j] = 0;
    ...
    for (let i = 0; i < ((cost.length * cost[0].length) + 2); i++) {
      path[i] = [];
    }
    ...
    while (!done) {
      switch (step) {
        case 1:
          step = hgStep1(step, cost);
          break;
        case 2:
          step = hgStep2(step, cost, mask, rowCover, colCover);
          break;
        case 3:
          step = hgStep3(step, mask, colCover);
          break;
        case 4:
          step = hgStep4(step, cost, mask, rowCover, colCover, zeroRC);
          break;
        case 5:
          step = hgStep5(step, mask, rowCover, colCover, zeroRC, path);
          break;
        case 6:
          step = hgStep6(step, cost, rowCover, colCover);
          break;
        case 7:
          done = true;
          break;
    ...
    // In an input matrix taller than it is wide, the first assignment column will
    // have to skip some numbers, so the index will not always match the first
    // column.
    assignmentsSeen = 0;
    for (let i = 0; i < mask.length; i++) {
      for (let j = 0; j < mask[i].length; j++) {
    ...
    if (returnSum === true) {
      // If you want to return the min or max sum instead of the assignment array, set
      // the returnSum argument (or use this code on the return value outside of this
      // function):
      let sum = 0;
